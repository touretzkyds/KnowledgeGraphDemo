...
function getNearestSubRegion(maps, name, normalizedX, normalizedY) {
  ...
  let [nearestRegion, concept] = ["", ""];
  for (let i = 0; i < data.length; i++) {
    let currRegion = data[i];
    let currDistance = distanceBetween(normalizedX, normalizedY, currRegion.x, currRegion.y);
...
function addSubRegion(evt, regionType, maps, toolKit) {
    ...
    let key = "has" + subRegionType;
    let value = subRegionName + '\n' + subRegionConcept
